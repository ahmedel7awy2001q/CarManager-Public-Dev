...
/** Single source of truth for system notifications raised from the local attention data. */
...
internal object AttentionNotificationPolicy {
        ...
        plans.asSequence()
            ...
            .forEach { plan ->
                ...
                val level = when {
                    ...
                    plan.priority >= 10 -> AppAlertLevel.URGENT
                    decision.level == MaintenancePriorityLevel.CRITICAL -> AppAlertLevel.URGENT
                    plan.priority >= 5 -> AppAlertLevel.IMPORTANT
                ...
                val priorityText = when {
                    plan.priority >= 10 -> "أولوية عالية"
                    plan.priority >= 5 -> "أولوية مهمة"
                    else -> null
                }
                val body = listOf(plan.titleAr, detail, priorityText, decision.reasonAr)
                    .filterNotNull()
                ...
                add(
                    AppAlert(
                        ...
                        body = body,
                        level = level,
                        fingerprint = "${status.urgency}:${decision.level}:${plan.priority}",
    ...
    private fun MutableList<AppAlert>.addFaultAlerts(vehicleName: String, faults: List<FaultRecordEntity>) {
        faults.asSequence()
            .filter { fault ->
                !fault.isDeleted &&
    ...
    private fun MutableList<AppAlert>.addDocumentAlerts(
        ...
        documents.asSequence()
            ...
            .filter {
                it.documentType == DocumentType.VEHICLE_LICENSE ||
                    it.documentType == DocumentType.INSURANCE ||
                    it.documentType == DocumentType.INSPECTION
            }
            .forEach { document ->
                ...
                val level = when {
                    state == "expired" -> AppAlertLevel.URGENT
                    document.documentType == DocumentType.VEHICLE_LICENSE && state == "week" -> AppAlertLevel.URGENT
                    state == "week" -> AppAlertLevel.IMPORTANT
                    document.documentType == DocumentType.VEHICLE_LICENSE -> AppAlertLevel.IMPORTANT
                ...
                add(
                    AppAlert(
                        ...
                        level = level,
                        fingerprint = "$state:${document.documentType}",
                        repeatAfterMs = repeatFor(level)
                    )
                )
            }
    ...
    private fun MutableList<AppAlert>.addReminderAlerts(
        ...
        reminders.asSequence()
            ...
            .forEach { reminder ->
                ...
                val level = when {
                    reminder.priority >= 10 || overdue && reminder.priority >= 5 -> AppAlertLevel.URGENT
                    reminder.priority >= 5 || overdue -> AppAlertLevel.IMPORTANT
                    else -> AppAlertLevel.REMINDER
                }
                val details = buildList {
                    ...
                    if (isEmpty() && reminder.priority >= 10) add("أولوية عالية")
                    else if (isEmpty() && reminder.priority >= 5) add("أولوية مهمة")
                }
                add(
                    AppAlert(
    ...
    private fun overdueDetail(overdueKm: Double?, overdueDays: Long?): String = when {
        overdueKm != null && overdueKm > 0.0 && overdueDays != null && overdueDays > 0L -> "متأخر ${formatKm(overdueKm)} كم و$overdueDays يوم"
    ...
    private fun dueSoonDetail(remainingKm: Double?, remainingDays: Long?, forecastDays: Long?): String = when {
        remainingKm != null && remainingKm >= 0.0 && remainingDays != null && remainingDays >= 0L -> "متبقي ${formatKm(remainingKm)} كم أو $remainingDays يوم"
